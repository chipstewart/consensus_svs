...
cmd = 'python /opt/src/algutil/firehose_module_adaptor/run_module.py --module_libdir /opt/src/MantaVCF2dRangerForBP  \
--VCF %s \
--pair_id %s \
--normpaneldb %s \
--use_PoN_JH true \
...
--min.tum.SR %s \
--min.tum.RP %s \
--min.tum %s '% (VCF,PAIRID,PONDIR,minscoreforbp,RMAT,minspan,maxpon,maxnorm,mintumorSR,mintumorRP,mintumor)
...
EOF
    ...
    output {
        File manta_forBP_txt="${PAIRID}.manta_results.forBP.txt"
        File mantavcf2drangerforbp_dstat_log="dstat.log"
    }

    runtime {
        docker : "docker.io/chipstewart/mantavcf2drangerforbp:1"
        memory: "${ram_gb}GB"
        cpu: "${cpu_cores}"
...
workflow mantavcf2dRangerForBP_workflow {
    call mantavcf2dRangerForBP
}
