task pcawg_snowmanvcf2dRangerForBP {
...
cmd = 'python /opt/src/algutil/firehose_module_adaptor/run_module.py --module_libdir /opt/src/pcawg_snowmanvcf2dRangerForBP \
--VCF %s \
--normpaneldb %s \
--use_PoN_JH true \
--minscoreforbp  %s \
...
if len(outfile)>0:
   outfile=outfile[0]
   cmd1='mv ' + outfile + ' ' + PAIRID +'.snowman_results.forBP.txt'
   run(cmd1)
...
EOF
    ...
    runtime {
        docker : "docker.io/chipstewart/pcawg_snowmanvcf2drangerforbp:1"
        memory: "${ram_gb}GB"
        cpu: "${cpu_cores}"
...
workflow pcawg_snowmanvcf2dRangerForBP_workflow {
    call pcawg_snowmanvcf2dRangerForBP
}
